...
point intersect(line s1, line s2){
    point ans;
    ans.x = -double(s1.c * s2.b - s2.c * s1.b) / double(s1.a * s2.b - s2.a *  s1.b);
    ans.y = -double(s1.a * s2.c - s2.a * s1.c) / double(s1.a * s2.b - s2.a *  s1.b);
    return ans;
}

coord dist(line s1, point p){
    return abs(s1.a * p.x + s1.b * p.y + s1.c) / hypot(s1.a, s1.b);
}
...
int32_t main()
{
    ...
    int a = next(), b = next(), c = next(), a1 = next(), b1 = next(), c1 = next();

    line s1(a, b, c), s2(a1, b1, c1);
